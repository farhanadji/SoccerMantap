...
open class TeamPresenter(val view: TeamView, val gson: Gson, val context: CoroutineContextProvider = CoroutineContextProvider()){

    fun getTeam(id: String){
        GlobalScope.launch(context.main){
            val teamData = gson.fromJson(ApiRepository().doRequestAsync(SportDBAPI.getTeamList(id)).await(), TeamResponse::class.java)
